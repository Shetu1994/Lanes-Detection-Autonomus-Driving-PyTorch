...
class TiltRoadFilter(ConfigFilter):
    ...
    def filter(self, config):
        ...
        #print(config['layer_params']['road'][1]);
        config['layer_params']['road'][1][1] += self.tilt;
        config['layer_params']['lanes'][0][1][1] += self.tilt;
        config['layer_params']['lanes'][1][1][1] += self.tilt;
        config['layer_params']['lanes'][2][1][1] += self.tilt;
        config['layer_params']['lanes'][3][1][1] += self.tilt;
        #print(config['layer_params']['road'][0][1]);
        return config
...
class ShiftRoadFilter(ConfigFilter):
    ...
    def __init__(self, lb, ub):
        ...
        ShiftRoadFilter.SHIFT = self.shift
        
    def filter(self, config):
        # Use drawn shift to shift road and lanes by modifying the coordinates
        # defined in the 'config'
        config['layer_params']['road'][0][1] += self.shift;
        config['layer_params']['road'][1][1] += self.shift;
        return config
...
class ShiftLanesFilter(ConfigFilter):
    ...
    def filter(self, config):
        ...
        #   Shift lane
        config['layer_params']['lanes'][0][0][1] += ShiftRoadFilter.SHIFT;
        config['layer_params']['lanes'][0][1][1] += ShiftRoadFilter.SHIFT;
        config['layer_params']['lanes'][1][0][1] += ShiftRoadFilter.SHIFT;
        config['layer_params']['lanes'][1][1][1] += ShiftRoadFilter.SHIFT;
        config['layer_params']['lanes'][2][0][1] += ShiftRoadFilter.SHIFT;
        config['layer_params']['lanes'][2][1][1] += ShiftRoadFilter.SHIFT;
        config['layer_params']['lanes'][3][0][1] += ShiftRoadFilter.SHIFT;
        config['layer_params']['lanes'][3][1][1] += ShiftRoadFilter.SHIFT;
...
class LaneWidthFilter(ConfigFilter):
    ...
    def filter(self, config):
        ...
        config['layer_params']['lane_widths'][0] += self.delta_width;
        config['layer_params']['lane_widths'][1] += self.delta_width;
        config['layer_params']['lane_widths'][2] += self.delta_width;
        config['layer_params']['lane_widths'][3] += self.delta_width;
        return config
