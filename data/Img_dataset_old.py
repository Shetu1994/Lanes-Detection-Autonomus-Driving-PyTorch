...
class Img_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(DEFAULT_DATA_PATH,self.dataset_name);
        img_path1 = os.path.join(img_path,self.img_dataset[idx]);
        img = io.imread(img_path1);
        ...
        if self.mode ==Mode.PREDICT :
            ...
            h5_path = os.path.join(img_path,self.detetions_file_name);
    ...
    def _init_dataset(self,dataset_name,seed,params):
        ...
        if(os.path.isdir(dir_path)):
            ...
            im_list = [name for name in os.listdir(dir_path) if name.endswith(('.jpg', '.jpeg', '.png'))]   
            im_num = len(im_list);
            if(im_num < self.size):
